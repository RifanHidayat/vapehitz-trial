<?php
class Reqretailtocentral_Service {
	...
	public function getmenu() {
        ...
        try {
            $query ="SELECT * from menu where source = 'reqretailtocentral' ";
	...
	public function getNoSeq() {
		...
		$curYear = date('Y');
        try {
            $query ="select max(seq) FROM reqretailtocentral where tahun = '$curYear'";
	...
	public function getdatabarangid($kode_barang) {
        ...
        try {
            $query ="select nama_barang, ket, nic, stok_pusat, on_hand, stok_retail FROM liquid where kode_barang = '$kode_barang'
					 union
					 select nama_device as nama_barang, ket, jenis_device, stok_pusat, on_hand, stok_retail FROM device where kode_device = '$kode_barang'
					 union
					 select nama_aksesoris as nama_barang, ket, jenis_aksesoris, stok_pusat, on_hand, stok_retail FROM accessories where kode_aksesoris = '$kode_barang'
					 union
					 select nama_atomizer as nama_barang, ket, jenis_atomizer, stok_pusat, on_hand, stok_retail FROM atomizer where kode_atomizer = '$kode_barang'
					 ";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
    ...
    public function getlist() {
        ...
        try {
            $query ="SELECT * from reqretailtocentral order by no_request ASC ";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function getDatareqretailtocentral($no_request_data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="SELECT * from reqretailtocentral where no_request = '$no_request_data'";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function getDataDetailreqretailtocentral($no_request_data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="SELECT * from reqretailtocentral_sub where no_request = '$no_request_data' order by kode_subrequest Asc";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function insertdata(array $data){
		...
		$db->insert('reqretailtocentral',$insdata);
		...
		if(count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> ''){
			...
			$db->insert('reqretailtocentral_sub',$insdata2);
	...
	public function editdata(array $data){
	   ...
	   $insdata = array("tgl_request" => $data['tgl_request']);
		...
		$db->update('reqretailtocentral',$insdata,$where);
		
		if(count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> ''){
			
			$db->delete('reqretailtocentral_sub', $where);
	...
	public function hapusData($dataInput){
		...
		$db->delete('reqretailtocentral', $where);
		$db->delete('reqretailtocentral_sub', $where);
		$db->commit();
	    return 'sukses';
	   } catch (Exception $e) {
