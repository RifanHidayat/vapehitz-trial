...
class Jenisatomizer_Service {
	...
	public function getmenu() {
        ...
        try {
            $query ="SELECT * from menu where source = 'jenisatomizer' ";
	...
	public function getNoSeq() {
        ...
        try {
            $query ="select max(seq) FROM jenisatomizer";
	...
	public function getDataLiquid($id_jenis) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="SELECT * from jenisatomizer where id_jenis = '$id_jenis'";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function insertdata(array $data){
		...
		$insdata = array("id_jenis" => $data['id_jenis'],
						 "kode_ket" => $data['kode_ket'],
						 "keterangan" => $data['keterangan'],
						 "seq" => $data['seq'],
						 "status" => $data['status']);
					
		$db->insert('jenisatomizer',$insdata);
		$db->commit();
	    return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function editdata(array $data){
	    ...
	    $insdata = array("kode_ket" => $data['kode_ket'],
						 "keterangan" => $data['keterangan'],
						 "status" => $data['status']);
						 
		$where = "id_jenis = '".$id_jenis."'";
					
		$db->update('jenisatomizer',$insdata,$where);
		$db->commit();
	    return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function hapusData($dataInput){
		...
		$id_jenis = $dataInput['id_jenis'];
		
	    $where = "id_jenis = '".$id_jenis."'";
				
		$db->delete('jenisatomizer', $where);
		$db->commit();
	    return 'sukses';
	   } catch (Exception $e) {
         $db->rollBack();
	...
	public function getSeq($id_id_jenis) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="select max(seq_id_jenis) FROM liquid where id_id_jenis = '$id_id_jenis'";
            $result = $db->fetchOne($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
