...
class Salestudio_Service
{
	...
	public function getmenu()
	{
		...
		try {
			$query = "SELECT * from menu where source = 'salestudio' ";
	...
	public function getNoSeq()
	{
		...
		try {
			$query = "select max(seq) FROM salestudio";
			$result = $db->fetchOne($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function getNoSeq2($kode_inv)
	{
		...
		try {
			$query = "select max(seq) FROM salestudio where kode_inv = '$kode_inv'";
			$result = $db->fetchOne($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function getdatabarangid($kode_barang)
	{
		...
		try {
			$query = "select nama_barang as nama_barang, ket, stok_studio, hj_whs FROM liquid where kode_barang = '$kode_barang'
					 union
					 select nama_device as nama_barang, ket, stok_studio, hj_whs FROM device where kode_device = '$kode_barang'
					 union
					 select nama_aksesoris as nama_barang, ket, stok_studio, hj_whs FROM accessories where kode_aksesoris = '$kode_barang'
					 union
					 select nama_atomizer as nama_barang, ket, stok_studio, hj_whs FROM atomizer where kode_atomizer = '$kode_barang'
					 ";
			$result = $db->fetchAll($query);
			return $result;
		} catch (Exception $e) {
	...
	public function getlistsalestudio()
	{
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');

		try {
			$query = "SELECT * from salestudio order by no_invoice ASC ";
			$result = $db->fetchAll($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function getDatasalestudio($no_invoice_data)
	{
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');

		try {
			$query = "SELECT * from salestudio where no_invoice = '$no_invoice_data'";
			$result = $db->fetchAll($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function getDataNoInvoiceRevisi($no_invoice_data)
	{
		...
		try {
			$query = "SELECT no_revisi as jml,no_invoice_original from salestudio where no_invoice='$no_invoice_data'";
			$result = $db->fetchAll($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function getDataDetailsalestudio($no_invoice_data)
	{
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');

		try {
			$query = "SELECT * from salestudio_sub where no_invoice = '$no_invoice_data' order by kode_subsale Asc";
			$result = $db->fetchAll($query);
			return $result;
		} catch (Exception $e) {
			echo $e->getMessage() . '<br>';
	...
	public function revisidata(array $data)
	{
		...
		try {
			...
			$where_transaksi = "id_table_original = '" . $data['no_invoice_original'] . "'AND nama_table='salestudio'";
			...
			$db->insert('salestudio', $insdata);
			$db->update('salestudio', $jml_revisi_update, $where);
			...
			if (count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> '') {
				for ($x = 0; $x < count($data['kode_barang']); $x++) {
					...
					$db->insert('salestudio_sub', $insdata2);

					$hitung_stok = $stok_studio - $qty;

					$insdata3 = array("stok_studio" => $hitung_stok);
	...
	public function editdata(array $data)
	{
		...
		try {
			...
			$db->update('salestudio', $insdata, $where);

			if (count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> '') {

				$db->delete('salestudio_sub', $where);
				...
				for ($x = 0; $x < count($data['kode_barang']); $x++) {
					...
					$hj_whs_baru  = $data['hj_whs_baru'][$x];
					$kode 			 = $data['kode'][$x];
