...
class Retursalestudio_Service {
	...
	public function getmenu() {
        ...
        try {
            $query ="SELECT * from menu where source = 'retursalestudio' ";
	...
	public function getNoSeq() {
        ...
        try {
            $query ="select max(seq) FROM retursalestudio";
	...
	public function getdatabarangid($kode_barang) {
        ...
        try {
            $query ="select nama_barang as nama_barang, merk_barang as ket, stok_studio as stok_gudang FROM liquid where kode_barang = '$kode_barang'
					 union
					 select nama_device as nama_barang, ket as ket, stok_studio as stok_gudang FROM device where kode_device = '$kode_barang'
					 union
					 select nama_aksesoris as nama_barang, ket as ket, stok_studio as stok_gudang FROM accessories where kode_aksesoris = '$kode_barang'
					 union
					 select nama_atomizer as nama_barang, merk_atomizer as ket, stok_studio as stok_gudang FROM atomizer where kode_atomizer = '$kode_barang'
					 ";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
	...
	public function getlistReturPenjualan() {
        ...
        try {
            $query ="select a.no_retur, a.no_invoice, a.tgl_retur, a.total_qty_retur, 
					 ...
					 from retursalestudio a, salestudio b
	...
	public function editdata(array $data){
		...
		if(count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> ''){
			...
			$insdata = array("stok_studio" => $stok_gudang);		
			$where = "$kode = '".$kode_barang."'";
			...
			$db->insert('retursalestudio_sub',$insdata3);
		...
		$db->update('salestudio',$insdata4,$where4);  */
	...
	public function getDataDetailReturPenjualan($no_retur_data) {
        ...
        try {
            $query ="SELECT * from retursalestudio_sub where no_retur = '$no_retur_data' order by no_subretur Asc";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
