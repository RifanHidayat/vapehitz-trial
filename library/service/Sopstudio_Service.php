...
class Sopstudio_Service {
	...
	public function getmenu() {
        ...
        try {
            $query ="SELECT * from menu where source = 'sopstudio' ";
	...
	public function getNoSeq() {
        ...
        try {
            $query ="select max(seq) FROM sop_studio";
	...
	public function getdatabarangid($kode_barang) {
        ...
        try {
            $query ="select nama_barang, kode_rasa, nic, volume, bad_stock FROM liquid where kode_barang = '$kode_barang'
					 union
					 select nama_device as nama_barang, kode_warna, jenis_device, stok_pusat as stok_studio, bad_stock FROM device where kode_device = '$kode_barang'
					 union
					 select nama_aksesoris as nama_barang, ket, jenis_aksesoris, stok_pusat as stok_studio, bad_stock FROM accessories where kode_aksesoris = '$kode_barang'
					 union
					 select nama_atomizer as nama_barang, kode_warna, jenis_atomizer, stok_pusat as stok_studio, bad_stock FROM atomizer where kode_atomizer = '$kode_barang'
					 ";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
    ...
    public function getlistSOP() {
        ...
        try {
            $query ="SELECT * from sop_studio order by no_sop ASC ";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function getDatasopstudio($no_sop_data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="SELECT * from sop_studio where no_sop = '$no_sop_data'";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function getDataDetailsopstudio($no_sop_data) {
        $registry = Zend_Registry::getInstance();
        $db = $registry->get('db');

        try {
            $query ="SELECT * from sop_studio_sub where no_sop = '$no_sop_data' order by kode_subsop Asc";
            $result = $db->fetchAll($query);
            return $result;
        } catch (Exception $e) {
            echo $e->getMessage() . '<br>';
	...
	public function insertdata(array $data){
		...
		if(count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> ''){
			...
			$db->insert('sop_studio_sub',$insdata2);
			
			$insdata3 = array("stok_studio" => $qty_after);
	...
	public function editdata(array $data){
	   ...
	   $insdata = array("tgl_sop" => $data['tgl_sop']);
		...
		$db->update('sop_studio',$insdata,$where);
		
		if(count($data['kode_barang']) <> 0 && trim($data['kode_barang'][0]) <> ''){
			
			$db->delete('sop_studio_sub', $where);
