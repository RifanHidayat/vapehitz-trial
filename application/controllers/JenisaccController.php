...
require_once 'service/Jenisacc_Service.php';

class JenisaccController extends Zend_Controller_Action {

    public function init() {
        $registry = Zend_Registry::getInstance();
        $this->auth = Zend_Auth::getInstance();
        $this->Home_Service = Home_Service::getInstance();
		$this->Jenisacc_Service = Jenisacc_Service::getInstance();
    ...
    public function indexAction() {
        ...
        $this->_helper->layout->setLayout('jenisacc-layout');
		
		$this->view->Jenisacc_Service = $this->Jenisacc_Service;
		
		$this->view->data = $this->Jenisacc_Service->getlistliquid();
		$this->view->menu = $this->Jenisacc_Service->getmenu();
		$this->view->seq  = $this->Jenisacc_Service->getNoSeq();
    }
	...
	public function tambahdataAction() {
		...
		$this->view->supplier=$this->Jenisacc_Service->getSupplier();
		$this->view->rasa=$this->Jenisacc_Service->getRasa();
		$this->view->seq=$this->Jenisacc_Service->getNoSeq();
    }
	...
	public function kirimdataAction() { 
		...
		$data = array('id_jenis' => $id_jenis,
					  ...
					  'status' => $status); 
		if($action == 'tambah'){
			$this->view->datainsert=$this->Jenisacc_Service->insertdata($data);
		} else {
			$this->view->datainsert=$this->Jenisacc_Service->editdata($data);
		}
   ...
   public function hapusdataAction(){
		...
		$hasil = $this->Jenisacc_Service->hapusData($dataInput);
	...
	public function editdataAction() {
		...
		$this->view->data=$this->Jenisacc_Service->getDataLiquid($id_jenis);
	...
	public function cekseqAction(){
		...
		$this->view->id_id_jenis = $this->Jenisacc_Service->getSeq($id_id_jenis);
	}
