...
require_once 'service/Jenisatomizer_Service.php';

class JenisatomizerController extends Zend_Controller_Action {

    public function init() {
        $registry = Zend_Registry::getInstance();
        $this->auth = Zend_Auth::getInstance();
        $this->Home_Service = Home_Service::getInstance();
		$this->Jenisatomizer_Service = Jenisatomizer_Service::getInstance();
    ...
    public function indexAction() {
        ...
        $this->_helper->layout->setLayout('jenisatomizer-layout');
		
		$this->view->Jenisatomizer_Service = $this->Jenisatomizer_Service;
		
		$this->view->data = $this->Jenisatomizer_Service->getlistliquid();
		$this->view->menu = $this->Jenisatomizer_Service->getmenu();
		$this->view->seq  = $this->Jenisatomizer_Service->getNoSeq();
    }
	...
	public function tambahdataAction() {
		...
		$this->view->supplier=$this->Jenisatomizer_Service->getSupplier();
		$this->view->rasa=$this->Jenisatomizer_Service->getRasa();
		$this->view->seq=$this->Jenisatomizer_Service->getNoSeq();
    }
	...
	public function kirimdataAction() { 
		...
		$data = array('id_jenis' => $id_jenis,
					  ...
					  'status' => $status); 
		if($action == 'tambah'){
			$this->view->datainsert=$this->Jenisatomizer_Service->insertdata($data);
		} else {
			$this->view->datainsert=$this->Jenisatomizer_Service->editdata($data);
		}
   ...
   public function hapusdataAction(){
		...
		$hasil = $this->Jenisatomizer_Service->hapusData($dataInput);
	...
	public function editdataAction() {
		...
		$this->view->data=$this->Jenisatomizer_Service->getDataLiquid($id_jenis);
	...
	public function cekseqAction(){
		...
		$this->view->id_id_jenis = $this->Jenisatomizer_Service->getSeq($id_id_jenis);
	}
